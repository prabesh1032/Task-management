...
<div class="container mx-auto">
    ...
    <form action="{{ route('tasks.update', $task) }}" method="POST" class="max-w-lg">
        ...
        <div class="mb-4">
            ...
            <input type="text" id="title" name="title" class="border rounded w-full p-2" value="{{ old('title', $task->title) }}" required>
            @error('title')
        ...
        <div class="mb-4">
            ...
            <textarea id="description" name="description" class="border rounded w-full p-2">{{ old('description', $task->description) }}</textarea>
            @error('description')
                <div class="text-red-600 mt-2 text-sm">
                    *{{ $message }}
                </div>
